...
type t = {
  ...
  binding_additions : binding_addition list;
}
...
let provider_for_module
    ~(importer_ic : import_context)
    ~(importer_source_file : string)
    ~(module_name : string)
    ~(source_file : string)
  : provider_info =
  match Import.lookup_provider_in_source importer_ic ~source_file:importer_source_file module_name with
  | Some provider -> provider
  | None ->
    Auxl.error None
      ("Internal error: missing provider info for module '" ^ module_name
       ^ "' while linking imported file '" ^ source_file ^ "'.\n")
...
let build
  ...
  let binding_additions_acc : binding_addition list ref = ref [] in
  ...
  let ensure_root_mapping (orig : string) : unit =
    ...
    then ()
      ...
      let module_of_src =
        ...
        if Filename.check_suffix base ".ott" then Filename.chop_suffix base ".ott" else base
      in
      let provider =
        provider_for_module
          ~importer_ic
          ~importer_source_file
          ~module_name:module_of_src
          ~source_file:src
      in
  ...
  List.iter (fun r ->
    ...
    List.iter (fun p ->
      ...
      binding_additions_acc :=
        {
          ba_local_name = local_pn;
          ba_origin_name = p.prod_name;
          ba_provider = provider;
          ba_visibility = Import_binding_internal;
          ba_kinds = [];
        }
