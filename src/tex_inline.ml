...
let clone_import_context (ic : import_context) : import_context =
  {
    ...
    ic_provider_info_by_module = Hashtbl.copy ic.ic_provider_info_by_module;
...
let empty_import_context () : import_context =
  {
    ic_direct_providers = [];
    ic_imported_files = [];
    ic_provider_info_by_module = Hashtbl.create 0;
...
let inline_imported_defn_bodies
  ...
  let modules_needing_bodies : StringSet.t =
    ...
    let mark_if_imported_and_bodyless (name : string) ~(is_empty : bool) =
      if is_empty then
        ...
        | Some binding -> needed := StringSet.add binding.bi_provider.pi_module !needed
        | None -> ()
    in
    List.iter (function
      | RDC dc ->
  ...
  let link_by_source_module : ((string * string), Module_link.t) Hashtbl.t =
    Hashtbl.create 16
  in
  Hashtbl.iter (fun source_file scope_info ->
    List.iter (fun site ->
      let m = site.isi_provider.pi_module in
      let key = (source_file, m) in
      if StringSet.mem m modules_needing_bodies && not (Hashtbl.mem link_by_source_module key) then
        ...
        | Some xd_mod ->
          ...
          in
          List.iter (Import.add_binding_if_absent ic)
  ...
  let link_for_local_name ~(provider_module : string) ~(local_name : string)
    : Module_link.t option =
    match Import.source_files_for_binding ic local_name with
    | source_file :: _ ->
      Hashtbl.find_opt link_by_source_module (source_file, provider_module)
    | [] -> None
  in
    ...
    | RDC dc ->
      let dc_defns' =
        List.map (fun d ->
          ...
          match Import.lookup_binding ic local_prod with
          | None -> d
          | Some binding ->
            ...
            (match Hashtbl.find_opt compiled_by_module m with
             ...
             | Some idx ->
               ...
               | Some (src_dc_w, src_d_w, rules) ->
                 let link =
                   link_for_local_name ~provider_module:m ~local_name:local_prod
                 in
                 let rules' =
                   List.map (function
                     ...
                     | PSR_Rule dr ->
                       ...
                       in
                       (* Record binding info for TeX-visible rule names. *)
                       Import.add_binding_if_absent ic
                         {
                           ba_local_name = dr'.drule_name;
                           ba_origin_name = dr.drule_name;
                           ba_provider = binding.bi_provider;
                           ba_visibility = Import_binding_internal;
                           ba_kinds = [];
                         };
                       PSR_Rule dr'
                   ) rules
