...
type provenance_entry = string * (string * string)
...
type module_index = {
  ...
  nested_provenance : provenance_entry list;
...
type resolver_state = {
  ...
  module_of_file : (string, string) Hashtbl.t;
...
type resolved_import = {
  ...
  provenance_entries : provenance_entry list;
}
...
type internalized_import_items = {
  items : raw_item list;
  transitive_only_names : StringSet.t;
  transitive_hidden_locals : StringSet.t;
  provenance_entries : provenance_entry list;
}
...
let create_resolver_state () = {
  ...
  module_of_file = Hashtbl.create 16;
  imported_file_list = [];
...
let lookup_binding ic name =
  Hashtbl.find_opt ic.ic_binding_info_by_local_name name

let lookup_provider ic module_name =
  Hashtbl.find_opt ic.ic_provider_info_by_module module_name
...
let provider_exact_key (provider : provider_info)
  ...
  (provider.pi_module, provider.pi_file, provider.pi_default_id, sort_homs provider.pi_homs)

let dedup_providers_exact (providers : provider_info list) : provider_info list =
  let seen :
      ((string * string * string * (string * string) list), unit) Hashtbl.t =
...
let binding_info_of_addition (addition : binding_addition) : binding_info =
  {
    bi_local_name = addition.ba_local_name;
    bi_origin_name = addition.ba_origin_name;
    bi_provider = addition.ba_provider;
    bi_visibility = addition.ba_visibility;
    bi_kinds = addition.ba_kinds;
  }
...
let lookup_binding_in_source ic ~source_file name =
  let from_scope (scope : scope_info) =
    List.find_map (fun site ->
      Option.map binding_info_of_addition
        (List.find_opt (fun addition -> addition.ba_local_name = name) site.isi_bindings)
    ) scope.si_direct_imports
  in
  match lookup_scope_info ic source_file with
  | Some scope ->
    (match from_scope scope with
     | Some binding -> Some binding
     | None -> lookup_binding ic name)
  | None -> lookup_binding ic name
...
let lookup_provider_in_source ic ~source_file module_name =
  let from_scope (scope : scope_info) =
    List.find_map (fun site ->
      if site.isi_provider.pi_module = module_name then Some site.isi_provider else None
    ) scope.si_direct_imports
  in
  match lookup_scope_info ic source_file with
  | Some scope ->
    (match from_scope scope with
     ...
     | None -> lookup_provider ic module_name)
  | None -> lookup_provider ic module_name
...
let source_files_for_binding ic name : string list =
  let files =
    Hashtbl.fold (fun source_file scope acc ->
      let found =
        List.exists (fun site ->
          List.exists (fun addition -> addition.ba_local_name = name) site.isi_bindings
        ) scope.si_direct_imports
      in
      if found then source_file :: acc else acc
    ) ic.ic_scope_info_by_source_file []
  in
  List.sort_uniq String.compare files

let display_name ic name =
  match lookup_binding ic name with
...
let ensure_provider
    (providers : (string, provider_info) Hashtbl.t)
    (provider : provider_info) : provider_info =
  match Hashtbl.find_opt providers provider.pi_module with
  | Some existing -> existing
  | None ->
    Hashtbl.replace providers provider.pi_module provider;
    provider

let add_binding_if_absent ic (addition : binding_addition) =
  if Hashtbl.mem ic.ic_binding_info_by_local_name addition.ba_local_name then ()
  else
    let provider = ensure_provider ic.ic_provider_info_by_module addition.ba_provider in
    Hashtbl.replace ic.ic_binding_info_by_local_name addition.ba_local_name
      {
        bi_local_name = addition.ba_local_name;
        bi_origin_name = addition.ba_origin_name;
...
let resolve_module_file ~base_dir ~search_paths ~loc module_name =
  let filename = module_name ^ ".ott" in
  let candidates =
    (Filename.concat base_dir filename)
    :: List.map (fun dir -> Filename.concat dir filename) search_paths
  in
...
let extract_hom_pairs (homs : raw_homomorphism list) : (string * string) list =
  List.filter_map (fun (hn, rhs, _l) ->
    (* Each import hom should be {{ backend ModuleId }} *)
    let strings = List.filter_map (fun hse ->
      match hse with
      | Raw_hom_string s -> Some (String.trim s)
      | _ -> None
    ) rhs in
    match strings with
    | [module_id] when module_id <> "" -> Some (hn, module_id)
    | _ -> None
  ) homs
...
let provider_info_for_direct_import (module_index : module_index)
  ...
  {
    module_index.provider with
    pi_homs = extract_hom_pairs rid.rid_homs;
    pi_loc = rid.rid_loc;
  }

let build_provider_info_by_module ~state ~(direct_providers : provider_info list)
  : (string, provider_info) Hashtbl.t =
  let tbl : (string, provider_info) Hashtbl.t = Hashtbl.create 16 in
  Hashtbl.iter (fun _canonical_path module_index ->
    Hashtbl.replace tbl module_index.provider.pi_module module_index.provider
  ) state.loaded_modules;
  List.iter (fun provider ->
    if not (Hashtbl.mem tbl provider.pi_module) then
      Hashtbl.replace tbl provider.pi_module provider
  ) direct_providers;
  tbl
...
let binding_visibility ~explicit_names (local_name : string)
  : import_binding_visibility =
  if StringSet.mem local_name explicit_names then
    Import_binding_explicit
  else if Import_naming.is_internal_transitive local_name then
...
let build_module_index ~canonical_path ~declared_module_name
    ~(expanded_items : raw_item list)
    ~(nested_provenance : provenance_entry list)
  : module_index =
  ...
  in
  let provider =
    make_provider_info ~module_name:declared_module_name ~canonical_path ~homs:[] ~loc:[]
  in
...
let build_explicit_scope_and_provenance ~rid ~(module_index : module_index)
  ...
  let provenance_entries = ref module_index.nested_provenance in
  let add_prov local_name orig_name =
    provenance_entries := (local_name, (rid.rid_module, orig_name)) :: !provenance_entries
  in
    ...
    | Export_syntax ->
      ...
      (match item.raw_item.rii_rename with
       | None ->
         List.iter (fun syn ->
           add_scope_name syn;
           add_prov syn item.resolved_name
         ) (synonyms_of_primary module_index.synonym_map item.resolved_name)
       | Some renamed ->
         add_scope_name renamed;
         add_prov renamed item.resolved_name)
    | Export_defnclass ->
      add_scope_name local_name;
      add_prov local_name item.resolved_name;
      ...
      (match Hashtbl.find_opt module_index.class_members item.resolved_name with
       | Some members ->
         List.iter (fun member_name ->
           ...
           add_prov member_name member_name
         ) members
       | None -> ())
    | Export_fundefnclass ->
      add_scope_name local_name;
...
let internalize_transitive_names ~state ~(module_index : module_index)
  ...
  in
  let provenance_entries = ref provenance_entries in
  ...
  let internalize_map =
    StringSet.fold (fun name acc ->
      ...
      let src_mod =
        match Hashtbl.find_opt state.module_of_file src with
        | Some module_name -> module_name
        | None -> fallback_module_name
      in
      let internal_name = internalize_name ~src ~name in
      (* Record provenance so backends can qualify references to these hidden
         transitive-only names (they are not declared in the importing file). *)
      provenance_entries := (internal_name, (src_mod, name)) :: !provenance_entries;
  ...
  in
  let transitive_hidden_locals =
    StringSet.of_list (List.map snd internalize_map)
  in
  {
    items;
    transitive_only_names;
    transitive_hidden_locals;
    provenance_entries = !provenance_entries;
  }

let namespace_imported_wrappers ~module_name
    ~(provenance_entries : provenance_entry list)
    (items : raw_item list)
  : raw_item list * provenance_entry list =
  (* Backend identifier namespacing for injected (imported) items.
     ...
     recording provenance entries that map the internal ids back to their
     original names. *)
  let provenance_by_local : (string, (string * string)) Hashtbl.t = Hashtbl.create 128 in
  List.iter (fun (local_name, (module_name', orig_name)) ->
    Hashtbl.replace provenance_by_local local_name (module_name', orig_name)
  ) provenance_entries;
  let prov_acc = ref [] in
  let add_prov_internal internal_name orig_name =
    if Hashtbl.mem provenance_by_local internal_name then ()
    else begin
      Hashtbl.replace provenance_by_local internal_name (module_name, orig_name);
      prov_acc := (internal_name, (module_name, orig_name)) :: !prov_acc
    end
  in
  let orig_of_local local_name =
    match Hashtbl.find_opt provenance_by_local local_name with
    | Some (_module_name, orig_name) -> orig_name
    | None -> local_name
  in
  let namespace_wrapper wrapper =
    if Import_naming.is_internal_wrapper wrapper then wrapper
    else Import_naming.wrapper_prefix_for_module ~module_name ^ wrapper
  in
  let items' =
    List.map (fun ri ->
      match ri with
        ...
        let raw_rs' =
          List.map (fun rr ->
            ...
            List.iter (fun rp ->
              ...
              add_prov_internal internal_prod orig_prod
            ) rr.raw_rule_ps;
      ...
      | Raw_item_dcs (Raw_RDC rdc) ->
        ...
        List.iter (fun rd ->
          ...
          add_prov_internal internal_judgement orig_judgement
        ) rdc.raw_dc_defns;
      ...
      | Raw_item_dcs (Raw_FDC rfdc) ->
        let fundefns' =
          List.map (fun rfd ->
            ...
            add_prov_internal internal_fn orig_fn_full;
  ...
  in
  (items', !prov_acc @ provenance_entries)

let build_site_bindings
    ~state
    ~(direct_provider : provider_info)
    ~(provenance_entries : provenance_entry list)
    ~(explicit_names : StringSet.t)
    ~(transitive_hidden_locals : StringSet.t)
  : binding_addition list =
  let provider_info_by_module =
    build_provider_info_by_module ~state ~direct_providers:[direct_provider]
  in
  let seen : (string, unit) Hashtbl.t = Hashtbl.create (List.length provenance_entries) in
  let additions_rev = ref [] in
  List.iter (fun (local_name, (source_mod, origin_name)) ->
    if not (Hashtbl.mem seen local_name) then begin
      Hashtbl.replace seen local_name ();
      let provider =
        if source_mod = direct_provider.pi_module then direct_provider
        else
          match Hashtbl.find_opt provider_info_by_module source_mod with
          | Some provider -> provider
          | None ->
            Auxl.error None
              ("Internal error: missing provider info for imported module '"
               ^ source_mod ^ "'.\n")
      in
      let visibility =
        if StringSet.mem local_name explicit_names then
          Import_binding_explicit
        else if StringSet.mem local_name transitive_hidden_locals then
          Import_binding_transitive_hidden
        else
          Import_binding_internal
      in
      additions_rev :=
        {
          ba_local_name = local_name;
          ba_origin_name = origin_name;
          ba_provider = provider;
          ba_visibility = visibility;
          ba_kinds = binding_kinds_for_origin ~state ~provider ~origin_name;
        }
        :: !additions_rev
    end
  ) provenance_entries;
  List.rev !additions_rev

let rec load_expanded_module_raw ~state ~base_dir ~search_paths
    ~keep_imported_defn_bodies ~loc ~module_name
  : module_index =
  let canonical_path =
    resolve_module_file ~base_dir ~search_paths ~loc module_name
  in
  Hashtbl.replace state.module_of_file canonical_path module_name;
  match Hashtbl.find_opt state.loaded_modules canonical_path with
    ...
    let loaded_module =
      try
        ...
        let child_provenance =
          List.filter (fun (name, _p) -> is_internal_prov_name name)
            resolved.provenance_entries
        in
        let expanded_items = resolved.imported_items @ resolved.local_items in
        let module_index =
          build_module_index
            ~canonical_path
            ~declared_module_name:module_name
            ~expanded_items
            ~nested_provenance:child_provenance
        in
...
and resolve_one_import ~state ~base_dir ~search_paths
  ...
  let explicit_view =
    build_explicit_scope_and_provenance
      ~rid
      ~module_index:loaded_module
      ~validated_items:validation
  in
  ...
  let internalized =
    internalize_transitive_names
      ~state
      ~module_index:loaded_module
      ~fallback_module_name:rid.rid_module
      ~scope_names:explicit_view.scope_names
      ~allowed_judgement_roots:explicit_view.allowed_judgement_roots
      ~provenance_entries:explicit_view.provenance_entries
      filtered_for_typechecking
  in
  let injected_items, provenance_entries =
    if keep_imported_defn_bodies then
      (internalized.items, internalized.provenance_entries)
    else
      namespace_imported_wrappers
        ~module_name:rid.rid_module
        ~provenance_entries:internalized.provenance_entries
        internalized.items
  in
  let direct_provider =
  ...
  let site_bindings =
    build_site_bindings
      ...
      ~direct_provider
      ~provenance_entries
      ~explicit_names:explicit_view.scope_names
      ~transitive_hidden_locals:internalized.transitive_hidden_locals
  in
  {
    injected_items;
    site_info = {
...
and resolve_source_file ~state ~base_dir ~search_paths ~keep_imported_defn_bodies
  ...
  let provenance_entries_acc = ref [] in
  ...
  List.iter (fun ri ->
    ...
    | Raw_item_import rid ->
      ...
      provenance_entries_acc :=
        !provenance_entries_acc @ resolved_import.provenance_entries
    | _ -> ()
  ) ris;
  ...
  in
  {
    ...
    provenance_entries = !provenance_entries_acc;
    explicit_names = !explicit_imported_names;
  }
...
let resolve_imports ~base_dirs ~search_paths ~keep_imported_defn_bodies
  ...
  let all_provenance = Hashtbl.create 64 in
  ...
  let explicit_imported_names_global = ref StringSet.empty in
  ...
  let add_provenance ~loc name (source_mod, orig_name) =
    match Hashtbl.find_opt all_provenance name with
    | None -> Hashtbl.replace all_provenance name (source_mod, orig_name)
    | Some (m, o) ->
      if m = source_mod && o = orig_name then ()
      else
        Auxl.error (Some loc)
          ("Inconsistent import provenance for name '" ^ name ^ "':\n"
           ^ "  previously: " ^ m ^ "." ^ o ^ "\n"
           ^ "  now:        " ^ source_mod ^ "." ^ orig_name ^ "\n")
  in
  ...
  let ris_per_file' = List.map2 (fun base_dir ris ->
    ...
    explicit_imported_names_global :=
      StringSet.union !explicit_imported_names_global resolved.explicit_names;
    let provenance_loc =
      match ris with
      | ri :: _ -> loc_of_item ri
      | [] -> dummy_loc
    in
    List.iter (fun (local_name, source_info) ->
      add_provenance ~loc:provenance_loc local_name source_info
    ) resolved.provenance_entries;
    let imported_items =
      suppress_items_for_tc_env
        ~claim_decl_group
  ...
  let provider_info_by_module =
    build_provider_info_by_module ~state ~direct_providers
  in
  let binding_info_by_local_name : (string, binding_info) Hashtbl.t =
    Hashtbl.create (Hashtbl.length all_provenance)
  in
  Hashtbl.iter (fun local_name (source_mod, orig_name) ->
    let provider =
      match Hashtbl.find_opt provider_info_by_module source_mod with
      | Some provider -> provider
      | None ->
        Auxl.error None
          ("Internal error: missing provider info for imported module '" ^ source_mod ^ "'.\n")
    in
    Hashtbl.replace binding_info_by_local_name local_name
      {
        bi_local_name = local_name;
        bi_origin_name = orig_name;
        bi_provider = provider;
        bi_visibility = binding_visibility ~explicit_names:!explicit_imported_names_global local_name;
        bi_kinds = binding_kinds_for_origin ~state ~provider ~origin_name:orig_name;
      }
  ) all_provenance;
  ...
  let ic = {
    ...
    ic_provider_info_by_module = provider_info_by_module;
