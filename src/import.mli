...
(** Return the location of a raw item. *)
val loc_of_item : Types.raw_item -> Types.loc
...
(** Look up provider info for a specific importing source file, by file path. *)
val lookup_provider_by_file_in_source :
  Types.import_context ->
  source_file:string ->
  string ->
  Types.provider_info option
